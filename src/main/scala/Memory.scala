...
class ITCM(val depth: Int, val initHex: String) extends Module {
    ...
    val initInst = "src/test/hex/inst/" + initHex
    loadMemoryFromFileInline(mem, initInst)

    val addrWidth = log2Ceil(depth)

    // 将字节地址转为 word 地址
    val wordAddr = Mux(reset.asBool, 0.U, io.bus.addrb(addrWidth+1, 2))

    // 两次读：wordAddr 和 wordAddr+1
    io.bus.inst0 := mem.read(wordAddr)
    io.bus.inst1 := mem.read(wordAddr + 1.U)
}
